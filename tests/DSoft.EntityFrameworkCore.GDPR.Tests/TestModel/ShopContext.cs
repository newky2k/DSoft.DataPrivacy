...
namespace DSoft.EntityFrameworkCore.GDPR.Tests.TestModel;
...
public sealed class ShopContext : DbContext
{
    ...
    public ShopContext(SqliteConnection connection, Action<GdprOptionsBuilder>? configure = null)
        : base(new DbContextOptionsBuilder<ShopContext>().UseSqlite(connection).UseGdpr(gdpr =>
        {
            gdpr.UseHashKey(HashKey);
            configure?.Invoke(gdpr);
        }).Options)
    {
    }
...
public sealed class ShopDatabase : IDisposable
{
    ...
    public ShopContext CreateContext(Action<GdprOptionsBuilder>? configure = null) => new(Connection, configure);
