...
public sealed class PersonalDataModel
{
    ...
    private PersonalDataModel(IModel model, int maxLinkDepth)
    {
        ...
        if (model.FindAnnotation(GdprAnnotationNames.ConventionsApplied) == null)
        {
            throw new InvalidOperationException(
                "The GDPR conventions have not run for this model. Call UseGdpr() when configuring the DbContext.");
        }
        ...
        foreach (var entity in Entities.Where(e => e.IsDataSubject || e.Links.Count > 0))
        {
            foreach (var (property, path) in AllProperties(entity.EntityType, Array.Empty<INavigation>()))
            {
                ...
                if (property.PropertyInfo != null && options.Ignore?.Invoke(property.PropertyInfo) == true)
                    continue;
                if (property.FindAnnotation(GdprAnnotationNames.Categories) != null || property.FindAnnotation(GdprAnnotationNames.NotPersonalData) != null)
                    continue;
    ...
    private static PersonalDataEntity Describe(IEntityType entityType)
    {
        ...
        {
            IsDataSubject = entityType.FindAnnotation(GdprAnnotationNames.IsDataSubject)?.Value is true,
            DataClass = entityType.FindAnnotation(GdprAnnotationNames.DataClass)?.Value as string,
            Erasure = (ErasureAction)(entityType.FindAnnotation(GdprAnnotationNames.Erasure)?.Value as int? ?? 0),
            Exemption = (ErasureExemption)(entityType.FindAnnotation(GdprAnnotationNames.Exemption)?.Value as int? ?? 0),
            ExemptionReason = entityType.FindAnnotation(GdprAnnotationNames.ExemptionReason)?.Value as string,
            Description = entityType.FindAnnotation(GdprAnnotationNames.Description)?.Value as string,
        };
        ...
        foreach (var (property, path) in AllProperties(entityType, Array.Empty<INavigation>()))
        {
            ...
            else if (property.FindAnnotation(GdprAnnotationNames.NotPersonalData) != null)
                notPersonal.Add(property);

            if (path.Count == 0)
            {
                if (property.FindAnnotation(GdprAnnotationNames.RetentionTrigger)?.Value is int trigger)
                    triggers[(RetentionTrigger)trigger] = property;
                if (property.FindAnnotation(GdprAnnotationNames.AnonymisedAt)?.Value is true)
    ...
    {
        if (property.FindAnnotation(GdprAnnotationNames.Categories)?.Value is not long categories)
            return null;

        return new PersonalDataPropertyClassification
        {
            Categories = (PersonalDataCategory)categories,
            DataClass = property.FindAnnotation(GdprAnnotationNames.DataClass)?.Value as string,
            Anonymisation = (AnonymisationMethod)(property.FindAnnotation(GdprAnnotationNames.Anonymisation)?.Value as int? ?? 0),
            Anonymiser = property.FindAnnotation(GdprAnnotationNames.Anonymiser)?.Value as string,
            Erasure = (ErasureAction)(property.FindAnnotation(GdprAnnotationNames.Erasure)?.Value as int? ?? 0),
            RetentionExemption = (ErasureExemption)(property.FindAnnotation(GdprAnnotationNames.Exemption)?.Value as int? ?? 0),
            Description = property.FindAnnotation(GdprAnnotationNames.Description)?.Value as string,
    ...
    private sealed class LinkFinder
    {
        ...
        private static DataSubjectLinkKind DeclaredKind(IForeignKey foreignKey)
        {
            ...
            {
                if (property.FindAnnotation(GdprAnnotationNames.SubjectLinkKind)?.Value is int kind)
