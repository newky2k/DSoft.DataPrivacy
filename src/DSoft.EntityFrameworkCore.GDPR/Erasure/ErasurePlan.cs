...
internal sealed class ErasurePlan
{
    ...
    public ErasurePlan(DbContext context, PersonalDataModel model, GdprOptions options)
    {
        _context = context;
        _model = model;
        _options = options;
    ...
    public PersonalDataEntity Describe(object row, IEntityType queried)
    {
        ...
        return _model.Find(entityType) ?? throw new InvalidOperationException($"'{entityType.DisplayName()}' is not described by the GDPR model.");
    }
