...
public sealed class PersonalDataOperations
{
    ...
    internal PersonalDataOperations(DbContext context)
    {
        ...
        var extension = context.GetService<IDbContextOptions>().FindExtension<GdprOptionsExtension>()
            ?? throw new InvalidOperationException("GDPR support is not enabled for this context. Call UseGdpr() when configuring it.");

        Options = extension.Options;
        Model = PersonalDataModel.For(context.Model, Options.MaxLinkDepth);
    }

    /// <summary>The options configured with <c>UseGdpr</c>.</summary>
