...
{
    public static object? Anonymise(AnonymisationMethod method, IEntityType entityType, IProperty property, object? value, DataPrivacyOptions options, string? anonymiser)
    {
        if (value == null)
            return null;
        ...
        switch (method)
        {
            ...
            case AnonymisationMethod.Hash:
                ...
                var hasher = options.Hasher
                    ?? throw new InvalidOperationException($"'{entityType.ShortName()}.{property.Name}' is hashed on erasure, but no key is configured. Call UseHashKey in UseDataPrivacy.");
            ...
            case AnonymisationMethod.Custom:
                ...
                if (custom == null)
                    throw new InvalidOperationException($"'{entityType.ShortName()}.{property.Name}' uses the anonymiser '{anonymiser}', which is not registered. Call AddAnonymiser in UseDataPrivacy.");
