...
namespace DSoft.DataPrivacy.EntityFrameworkCore.Design;

/// <summary>
/// Keeps the data privacy annotations out of migration snapshots and designer files. They describe how data is handled,
/// not the schema, so migrations never need them. The package registers this for you through
/// <see cref="DesignTimeServicesReferenceAttribute"/>; set the MSBuild property <c>DataPrivacyDesignTimeServices</c>
/// to <c>false</c> to keep the annotations in snapshots instead.
/// </summary>
public sealed class DataPrivacyDesignTimeServices : IDesignTimeServices
{
    ...
    /// <summary>Forwards every call to the provider's generator, dropping data privacy annotations from what it keeps.</summary>
    public class AnnotationFilter : DispatchProxy
    {
        ...
        protected override object? Invoke(MethodInfo? targetMethod, object?[]? args)
        {
            ...
            if (targetMethod.Name == "FilterIgnoredAnnotations" && result is IEnumerable<IAnnotation> annotations)
                return annotations.Where(a => !a.Name.StartsWith(PrivacyAnnotationNames.Prefix, StringComparison.Ordinal)).ToList();
